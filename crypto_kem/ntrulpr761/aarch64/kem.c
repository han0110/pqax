...
static void Generator(Fq *G, const unsigned char *pk) {
    ...
    PQCLEAN_NTRULPR761_CLEAN_crypto_stream_aes256ctr((unsigned char *)L, 4 * p,
                                                     aes_nonce, pk);
    crypto_decode_pxint32(L, (unsigned char *)L);
    for (i = 0; i < p; ++i) {
...
static void Hide(unsigned char *c, unsigned char *r_enc, const Inputs r,
    ...
    Inputs_encode(r_enc + 1, r);
    {
        ...
        PQCLEAN_NTRULPR761_CLEAN_crypto_stream_aes256ctr((unsigned char *)L,
                                                         4 * p, aes_nonce, h);
        crypto_decode_pxint32(L, (unsigned char *)L);
        Short_fromlist(b, L);
    }
...
int crypto_kem_keypair(unsigned char *pk, unsigned char *sk) {
    ...
    Round_and_encode(pk + Seeds_bytes, aG);
    {
        ...
        Hash(sk + SecretKeys_bytes + PublicKeys_bytes + Inputs_bytes,
...
int crypto_kem_enc(unsigned char *c, unsigned char *k,
...
int crypto_kem_dec(unsigned char *k, const unsigned char *c,
    ...
    int i;
    {
        ...
        }
        {
            ...
            for (i = 0; i < I; ++i) {
                r[i] = (int8)-int16_negative_mask(
                    Fq_freeze(Right(T[i]) - aB[i] + 4 * w + 1));
            }
        }
    }
    {
