...
static int32_t goods_omegas[GOODS_N_HALF] = {
...
static int32_t goods_inv_omegas[GOODS_N_HALF] = {
...
// idea from `fqmul` in
// https://github.com/mupq/pqax/blob/main/crypto_kem/kyber768/aarch64/neon_ntt
...
#define cooley_tukey_no_mul_x4(x, t1, t2)                       \
    do {                                                        \
        t1.val[0] = x[i + size].val[0];                         \
        t1.val[1] = x[i + size].val[1];                         \
        t1.val[2] = x[i + size].val[2];                         \
        t1.val[3] = x[i + size].val[3];                         \
        x[i + size].val[0] = vsubq_s32(x[i].val[0], t1.val[0]); \
        x[i + size].val[1] = vsubq_s32(x[i].val[1], t1.val[1]); \
        x[i + size].val[2] = vsubq_s32(x[i].val[2], t1.val[2]); \
        x[i + size].val[3] = vsubq_s32(x[i].val[3], t1.val[3]); \
...
static int16_t barrett_q(int32_t src) {
    ...
    const int32_t m1 = ((1 << k1) + q / 2) / q;
    const int32_t m2 = ((1 << k2) + q / 2) / q;

    src -= q * ((m1 * src) >> k1);
    return src - q * ((m2 * src + (1 << (k2 - 1))) >> k2);
}

static void goods_ntt_512_x3(int32x4x4_t x[3][32]) {
    int32x4x4_t tx[3][32];
    int32x4x4_t t1, t2;
    ...
    for (int i = 0; i < 16; ++i) {
        const int size = 16;
        cooley_tukey_no_mul_x4(x[0], t1, t2);
        cooley_tukey_no_mul_x4(x[1], t1, t2);
        cooley_tukey_no_mul_x4(x[2], t1, t2);
    }
...
static void goods_schoolbook(int32x4x4_t x[3][32], int32x4x4_t y[3][32]) {
    int32x4x4_t t1, t2;
    int64x2x2_t t3;
    for (int i = 0; i < 32; ++i) {
...
static void goods_intt_512_x3(int32x4x4_t x[3][32]) {
    int32x4x4_t tx[3][32];
    int32x4x4_t t1, t2;
...
void mul_small_goods(Fq *h, const Fq *f, const small *g) {
    ...
    // mod poly
    for (int i = GOODS_P - 1; i >= p; --i) {
        hh[i - p] = mont_mul(hh[i - p] + hh[i], GOODS_MONT_R_MOD_Q);
        hh[i - p + 1] = mont_mul(hh[i - p + 1] + hh[i], GOODS_MONT_R_MOD_Q);
    }
...
void mul_small_goods_x2(Fq *h1, Fq *h2, const Fq *f1, const Fq *f2,
                        const small *g) {
    int32_t pf1[3][GOODS_N] = {0}, pf2[3][GOODS_N] = {0}, pg[3][GOODS_N] = {0};
    ...
    // load
    int32x4x4_t vpf1[3][32], vpf2[3][32], vpg[3][32];
    for (int i = 0; i < 32; ++i) {
        vpf1[0][i] = vld4q_s32(&pf1[0][i * 16]);
        vpf1[1][i] = vld4q_s32(&pf1[1][i * 16]);
        vpf1[2][i] = vld4q_s32(&pf1[2][i * 16]);
    }
    for (int i = 0; i < 32; ++i) {
        vpf2[0][i] = vld4q_s32(&pf2[0][i * 16]);
        vpf2[1][i] = vld4q_s32(&pf2[1][i * 16]);
        vpf2[2][i] = vld4q_s32(&pf2[2][i * 16]);
    }
    for (int i = 0; i < 32; ++i) {
        vpg[0][i] = vld4q_s32(&pg[0][i * 16]);
        vpg[1][i] = vld4q_s32(&pg[1][i * 16]);
    ...
    // ntt
    goods_ntt_512_x3(vpf1);
    goods_ntt_512_x3(vpf2);
    goods_ntt_512_x3(vpg);

    // goods_schoolbook
    goods_schoolbook(vpf1, vpg);
    goods_schoolbook(vpf2, vpg);

    // intt
    goods_intt_512_x3(vpf1);
    goods_intt_512_x3(vpf2);

    // store
    for (int i = 0; i < 32; ++i) {
        vst4q_s32(&pf1[0][i * 16], vpf1[0][i]);
        vst4q_s32(&pf1[1][i * 16], vpf1[1][i]);
        vst4q_s32(&pf1[2][i * 16], vpf1[2][i]);
    }
    for (int i = 0; i < 32; ++i) {
        vst4q_s32(&pf2[0][i * 16], vpf2[0][i]);
        vst4q_s32(&pf2[1][i * 16], vpf2[1][i]);
        vst4q_s32(&pf2[2][i * 16], vpf2[2][i]);
    }

    // unpermutate
    int32_t hh1[GOODS_P], hh2[GOODS_P];
    for (int i = 0; i < 3; ++i) {
        for (int j = 0; j < GOODS_N; ++j) {
            int k = (1024 * i + 513 * j) % GOODS_P;
            hh1[k] = pf1[i][j];
    ...
    // mod poly
    for (int i = GOODS_P - 1; i >= p; --i) {
        hh1[i - p] = mont_mul(hh1[i - p] + hh1[i], GOODS_MONT_R_MOD_Q);
        hh1[i - p + 1] = mont_mul(hh1[i - p + 1] + hh1[i], GOODS_MONT_R_MOD_Q);
    }
    for (int i = GOODS_P - 1; i >= p; --i) {
        hh2[i - p] = mont_mul(hh2[i - p] + hh2[i], GOODS_MONT_R_MOD_Q);
        hh2[i - p + 1] = mont_mul(hh2[i - p + 1] + hh2[i], GOODS_MONT_R_MOD_Q);
    }

    // mod q
    for (int i = 0; i < p; ++i) {
        h1[i] = barrett_q(centered_goods_q(hh1[i]));
    }
    for (int i = 0; i < p; ++i) {
        h2[i] = barrett_q(centered_goods_q(hh2[i]));
    }
}
